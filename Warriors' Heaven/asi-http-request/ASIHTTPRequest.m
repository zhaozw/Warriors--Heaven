...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]] self];	
		ASIAuthenticationError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]] self];
		ASIRequestCancelledError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]] self];
		ASIUnableToCreateRequestError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]] self];
		ASITooMuchRedirectionError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]] self];	
...
{
	return [[[self alloc] initWithURL:newURL] self];
}
...
- (void)setupPostBody
{
	if ([self shouldStreamPostDataFromDisk]) {
		...
		if (![self postBodyWriteStream]) {
			[self setPostBodyWriteStream:[[[NSOutputStream alloc] initToFileAtPath:[self postBodyFilePath] append:NO] self]];
			[[self postBodyWriteStream] open];
		}
	} else {
		if (![self postBody]) {
			[self setPostBody:[[[NSMutableData alloc] init] self]];
		}
	}	
}
...
{
	[self setupPostBody];
	NSInputStream *stream = [[[NSInputStream alloc] initWithFileAtPath:file] self];
	[stream open];
	int bytesRead;
...
- (NSString *)responseString
{
	...
	return [[[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:[self responseEncoding]] self];
}
...
- (void)startRequest
{
	...
	[self setAuthenticationLock:[[[NSConditionLock alloc] initWithCondition:1] self]];
	...
	if (![self downloadDestinationPath]) {
		[self setRawResponseData:[[[NSMutableData alloc] init] self]];
    }
	...
	if (![self proxyHost] && ![self proxyPort]) {
		...
		// Detect proxy settings and apply them	
			...
			#else
			NSDictionary *proxySettings = [(__bridge NSDictionary *)CFNetworkCopySystemProxySettings() self];
			#endif
			#else
			NSDictionary *proxySettings = [(NSDictionary *)SCDynamicStoreCopyProxies(NULL) autorelease];
			#endif

			proxies = [(__bridge NSArray *)CFNetworkCopyProxiesForURL((__bridge CFURLRef)[self url], (__bridge CFDictionaryRef)proxySettings) self];
...
//			[pool release];
		...
		[self setTotalBytesSent:[[(__bridge NSNumber *)CFReadStreamCopyProperty(readStream, kCFStreamPropertyHTTPRequestBytesWrittenCount) self] unsignedLongLongValue]];
...
- (BOOL)readResponseHeadersReturningAuthenticationFailure
{
	...
	if (CFHTTPMessageIsHeaderComplete(headers)) {
		...
		[self setResponseStatusMessage:[(__bridge NSString *)CFHTTPMessageCopyResponseStatusLine(headers) self]];
		...
		if (!isAuthenticationChallenge) {
			...
			if (cLength) {
				[self setContentLength:CFStringGetIntValue((__bridge CFStringRef)cLength)];
				if ([self mainRequest]) {
...
{
	NSMutableDictionary *newCredentials = [[[NSMutableDictionary alloc] init] self];
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		...
		[self setProxyAuthenticationScheme:[(NSString *)CFHTTPAuthenticationCopyMethod(proxyAuthentication) self]];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(proxyAuthentication)) {
		[self setProxyAuthenticationRealm:[(NSString *)CFHTTPAuthenticationCopyRealm(proxyAuthentication) self]];
	}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
		...
		CFRelease(responseHeader);
		[self setAuthenticationScheme:[(NSString *)CFHTTPAuthenticationCopyMethod(requestAuthentication) self]];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		[self setAuthenticationRealm:[(NSString *)CFHTTPAuthenticationCopyRealm(requestAuthentication) self]];
	}
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self downloadDestinationPath]) {
			if (![self fileDownloadOutputStream]) {
				...
				[self setFileDownloadOutputStream:[[[NSOutputStream alloc] initToFileAtPath:[self temporaryFileDownloadPath] append:append] self]];
...
{
	NSError *underlyingError = [(NSError *)CFReadStreamCopyError(readStream) self];
...
{
	NSURLProtectionSpace *protectionSpace = [[[NSURLProtectionSpace alloc] initWithHost:host port:port protocol:protocol realm:realm authenticationMethod:NSURLAuthenticationMethodDefault] self];
...
{
	NSURLProtectionSpace *protectionSpace = [[[NSURLProtectionSpace alloc] initWithProxyHost:host port:port type:NSURLProtectionSpaceHTTPProxy realm:realm authenticationMethod:NSURLAuthenticationMethodDefault] self];
	[[NSURLCredentialStorage sharedCredentialStorage] setDefaultCredential:credentials forProtectionSpace:protectionSpace];
}
...
{
	if (!sessionCookies) {
		[ASIHTTPRequest setSessionCookies:[[[NSMutableArray alloc] init] self]];
	}
...
+ (void)setSessionCookies:(NSMutableArray *)newSessionCookies
{
	...
	[sessionCookies self];
	sessionCookies = [newSessionCookies self];
...
+ (NSArray *)proxiesForURL:(NSURL *)theURL fromPAC:(NSURL *)pacScriptURL
{
	...
	NSArray *proxies = [(NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)theURL, &err2) self];
	if (err2) {
		return nil;
	}
#endif
...
+ (void)recordBandwidthUsage
{
	...
	[bandwidthMeasurementDate self];
	bandwidthMeasurementDate = [[NSDate dateWithTimeIntervalSinceNow:1] self];
...
+ (NSString*)base64forData:(NSData*)theData {
    ...
    return [[[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding] self];
}
